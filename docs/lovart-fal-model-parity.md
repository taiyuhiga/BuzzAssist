...
**fal.aiに存在しない**（Midjourneyは2026年時点でも公式APIなし）。Lovartは独自ルートでMidjourneyを提供しており、これは**Lovartでしか使えない付加価値**。fal.ai対応表の対象外。過去生成物で1:1指示→1024×1024の反映実績あり。パラメータ的な保証は一切なし（全て△）。
