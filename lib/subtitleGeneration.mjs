...
const SUBTITLE_CLOUD_DIRECT_AUDIO_MAX_BYTES = 15 * 1024 * 1024;
...
async function requestSubtitleGeneration({ audioPath, mimeType, fileName, model, requestId, reservationToken, scriptText }) {
  const buffer = await readFile(audioPath);
  const form = new FormData();
  form.append("model", model);
  form.append("requestId", requestId || "");
  form.append("reservationToken", reservationToken || "");
  if (buffer.byteLength > SUBTITLE_CLOUD_DIRECT_AUDIO_MAX_BYTES) {
...
function runCommandQuiet(command, args, timeoutMs = 600_000) {
  return new Promise((resolve, reject) => {
    let stderr = "";
    let stdout = "";
    let child;
    ...
    child.on("close", (code) => {
      ...
      else reject(new Error(stderr.trim().slice(-400) || `${basename(command)} exited with code ${code}`));
    });
  });
}

async function resolveExecutable(envVar, name) {
  const fromEnv = nonEmptyString(process.env[envVar]);
  ...
  return name; // fall back to PATH resolution; spawn ENOENT gives the error
}

// BGM is the single biggest killer of ASR accuracy and word timing. When
// requested, split the vocals out with Demucs before transcription.
export async function separateVocalsForTranscription(audioPath) {
  const demucs = await resolveExecutable("DEMUCS_PATH", "demucs");
  const outDir = join(tmpdir(), `codex-excalidraw-demucs-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`);
  try {
    await runCommandQuiet(demucs, ["--two-stems=vocals", "-n", "htdemucs", "-o", outDir, audioPath], 1_800_000);
  } catch (error) {
    if (error?.code === "ENOENT") {
      throw new Error(
        "BGM分離にはDemucsが必要です。`pip3 install demucs` でインストールするか、DEMUCS_PATH でバイナリを指定してください。",
      );
    }
    throw new Error(`Demucs vocal separation failed: ${getErrorMessage(error)}`);
  }
  const stem = basename(audioPath, extname(audioPath));
  const vocalsPath = join(outDir, "htdemucs", stem, "vocals.wav");
  try {
    await access(vocalsPath);
  } catch {
    await rm(outDir, { recursive: true, force: true }).catch(() => {});
    throw new Error("Demucs finished but produced no vocals stem.");
  }
  return {
    path: vocalsPath,
...
// Local re-alignment: run WhisperX (phoneme-level wav2vec2 alignment) and snap
// each cue boundary to the nearest locally-aligned word boundary. Cloud word
// timestamps are good; this tightens them to ±tens of milliseconds.
export async function realignSubtitleLinesLocally(audioPath, subtitleLines) {
  const whisperx = await resolveExecutable("WHISPERX_PATH", "whisperx");
  const outDir = join(tmpdir(), `codex-excalidraw-whisperx-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`);
  await mkdir(outDir, { recursive: true });
  const model = nonEmptyString(process.env.WHISPERX_MODEL) || "small";
  try {
    await runCommandQuiet(
      whisperx,
      [audioPath, "--language", "ja", "--model", model, "--compute_type", "int8", "--output_format", "json", "--output_dir", outDir],
      1_800_000,
    );
  } catch (error) {
    await rm(outDir, { recursive: true, force: true }).catch(() => {});
    if (error?.code === "ENOENT") {
      throw new Error(
        "ローカル再アライメントにはWhisperXが必要です。`pip3 install whisperx` でインストールするか、WHISPERX_PATH でバイナリを指定してください。",
      );
    }
    throw new Error(`WhisperX alignment failed: ${getErrorMessage(error)}`);
  }
  let alignedWords = [];
  try {
    const files = (await readdir(outDir)).filter((file) => file.endsWith(".json"));
    if (files.length === 0) throw new Error("WhisperX produced no JSON output.");
    const payload = JSON.parse(await readFile(join(outDir, files[0]), "utf8"));
  ...
  alignedWords.sort((a, b) => a.start - b.start);
  const starts = alignedWords.map((word) => word.start);
  const ends = alignedWords.map((word) => word.end);
  const snap = (value, pool, tolerance) => {
    let best = value;
  ...
  const result = [];
  for (const cue of subtitleLines) {
    let start = snap(cue.start, starts, 0.35);
    let end = snap(cue.end, ends, 0.35);
    const previous = result[result.length - 1];
    if (previous && start < previous.end + 0.01) start = previous.end + 0.01;
    if (end < start + 0.2) end = Math.max(cue.end, start + 0.2);
    result.push({ ...cue, start, end });
  }
  return result;
}
...
// Quiet recordings transcribe noticeably worse. Measure the mean volume and
// loudness-normalize the upload copy when the source sits below -30 dB.
export async function prepareAudioForTranscription(audioPath) {
  let meanVolume = null;
  try {
    const stderr = await runFfmpegQuiet(["-hide_banner", "-i", audioPath, "-af", "volumedetect", "-f", "null", "-"]);
    const match = stderr.match(/mean_volume:\s*(-?\d+(?:\.\d+)?)\s*dB/);
    meanVolume = match ? Number(match[1]) : null;
  } catch {
    // volumedetect is metadata only — normalization below still applies
  }
  try {
    // Always normalize: consistent loudness plus a high-pass that strips
    // low-frequency rumble measurably improves ASR word accuracy and word
    // timestamps (previously only ran when mean volume was under -30dB).
    ...
    await runFfmpegQuiet(["-y", "-i", audioPath, "-af", "highpass=f=80,loudnorm=I=-16:TP=-1.5:LRA=11", "-ar", "44100", "-b:a", "128k", normalizedPath]);
    return {
      path: normalizedPath,
      normalized: true,
      meanVolume,
    ...
    return { path: audioPath, normalized: false, meanVolume };
  }
}
...
export async function transcribeAudioWords(options = {}) {
  ...
  let payload;
  try {
    payload = await requestSubtitleGeneration({
...
export async function generateSubtitleSrt(options = {}) {
  ...
  // Optional Demucs pass: transcribe the vocals stem instead of the mix.
  let vocalSeparation = null;
  if (options.separateVocals === true) {
    vocalSeparation = await separateVocalsForTranscription(audioPath);
  }
  const transcriptionSource = vocalSeparation?.path || audioPath;
  // Video sources always go through ffmpeg so the audio track gets extracted.
  const prepared = options.normalizeAudio === false && !isVideoSourceFile(transcriptionSource)
    ? { path: transcriptionSource, normalized: false }
    : await prepareAudioForTranscription(transcriptionSource);
  ...
  // Optional WhisperX pass: snap cue boundaries to locally-aligned
  // phoneme-level word times (±tens of ms).
  let locallyRealigned = false;
  if (options.localRealign === true) {
    try {
      subtitleLines = await realignSubtitleLinesLocally(transcriptionSource, subtitleLines);
      locallyRealigned = true;
    } catch (error) {
      qualityIssues = [...qualityIssues, { index: -1, type: "local_realign_failed", detail: getErrorMessage(error) }];
    }
  }
  ...
  try {
    const speechRanges = await detectSpeechRanges(transcriptionSource, durationSeconds);
  ...
  return {
    ...
    vocalsSeparated: Boolean(vocalSeparation),
    locallyRealigned,
    quality: {
