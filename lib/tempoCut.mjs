...
export async function nextGeneratedSilenceCutName(outputDir, additionalNames = []) {
  return nextNumberedGeneratedFileName({
    directory: outputDir,
    prefix: "JetCut",
    extension: ".xml",
    pattern: /^JetCut(\d+)(?!\d)(?:\s+\(\d+\)|-\d+)?\./i,
    additionalNames,
  });
}
...
export async function silenceCutVideo(options) {
  ...
  try {
    ...
    const desiredFileName = normalizeOutputFileName(opts.fileName) || (await nextGeneratedSilenceCutName(outputDir, opts.existingFileNames));
