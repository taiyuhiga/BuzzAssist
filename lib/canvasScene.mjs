...
// Older SRT cards were written with transparent fills, which Excalidraw
// only hit-tests on the stroke — making them impossible to drag. Re-color
// them to the BuzzAssist card chrome once.
export async function normalizeSubtitleCards(args = {}) {
  const scene = await loadScene(args);
  let changed = 0;
  scene.elements = scene.elements.map((element) => {
    if (
      !element?.customData?.codexGeneratedSubtitle ||
      element.isDeleted ||
