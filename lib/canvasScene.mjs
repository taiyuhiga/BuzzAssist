...
// safeOnly (the default for server startup) runs health and consistency tasks:
// externalizing inline base64, removing .tmp files, and reflecting files that
// were deleted while the server was offline. It intentionally SKIPS tasks that
...
function silenceCutAttachmentDataURL({ width, height, fileName, inputDuration, outputDuration, cutCount }) {
  const safeWidth = Math.max(1, Math.round(Number(width) || 320));
  const safeHeight = Math.max(1, Math.round(Number(height) || 180));
  const title = escapeSvgText(truncateMiddle(fileName || "jetcut.xml", 38));
  const hasDurations = Number.isFinite(Number(inputDuration)) && Number.isFinite(Number(outputDuration));
  const durationText = hasDurations
    ? `${formatShortDuration(inputDuration)} → ${formatShortDuration(outputDuration)}`
    : "Premiere Pro XML";
  const count = Number(cutCount);
  const detail = escapeSvgText(`${durationText}${Number.isFinite(count) ? ` · ${count}箇所` : ""}`);
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${safeWidth}" height="${safeHeight}" viewBox="0 0 320 180"><rect width="320" height="180" fill="#ffffff"/><rect x="22" y="24" width="74" height="74" rx="18" fill="#7c3aed" opacity="0.12"/><g fill="none" stroke="#7c3aed" stroke-width="4.2" stroke-linecap="round" stroke-linejoin="round"><path d="M54 48l-14 13 14 13"/><path d="M72 48l14 13-14 13"/><path d="M66 43l-8 38"/></g><text x="116" y="50" font-family="Inter,Arial,sans-serif" font-size="13" font-weight="700" fill="#7c3aed">Premiere XML</text><text x="116" y="76" font-family="Inter,Arial,sans-serif" font-size="17" font-weight="700" fill="#1f2937">${title}</text><text x="116" y="103" font-family="Inter,Arial,sans-serif" font-size="13" fill="#8a94a6">${detail}</text><path d="M24 148H296" stroke="#edf0f5" stroke-width="2"/></svg>`;
  // Keep the viewBox fixed so cards can match a replaced generator's bounds
  // without losing the same proportions as manually attached XML files.
  return `data:image/svg+xml;base64,${Buffer.from(svg).toString("base64")}`;
}
...
export async function insertExcalidrawSilenceCutResult(args = {}) {
  ...
  const anchorBounds = anchorElement ? elementBounds(anchorElement) : null;
  const matchAnchor = args.matchAnchor === true && anchorBounds;
  const width = finiteNumber(args.displayWidth, matchAnchor ? anchorBounds.width : 320);
  const height = finiteNumber(args.displayHeight, matchAnchor ? anchorBounds.height : 180);
  const margin = Math.max(0, finiteNumber(args.margin, 40));
  const replaceAnchor = Boolean(args.replaceAnchor && anchorElement);
  const placement = replaceAnchor ? "replace" : (["right", "left", "below", "replace", "inside"].includes(args.placement) ? args.placement : "below");
  const bounds = choosePlacement({ scene, anchorElement, width, height, margin, placement });
  ...
  const resultElement = newImageElementRecord({
    id: elementId,
    fileId,
    index,
    bounds,
  ...
  const fileRecord = {
    ...
    dataURL: silenceCutAttachmentDataURL({
      width: bounds.width,
      height: bounds.height,
      fileName,
      inputDuration: args.inputDuration,
      outputDuration: args.outputDuration,
      cutCount: args.cutCount,
    }),
    created: Date.now(),
    lastRetrieved: Date.now(),
  };
