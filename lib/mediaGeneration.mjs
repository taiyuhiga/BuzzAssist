...
import {
  RUNWAY_IMAGE_MODELS,
  RUNWAY_VIDEO_MODELS,
  generateRunwayImageMedia,
  generateRunwayVideoMedia,
  isRunwayImageModel,
  isRunwayVideoModel,
} from "./runwayMediaGeneration.mjs";
import {
  HIGGSFIELD_IMAGE_MODELS,
  HIGGSFIELD_VIDEO_MODELS,
  generateHiggsfieldImageMedia,
  generateHiggsfieldVideoMedia,
  isHiggsfieldImageModel,
  isHiggsfieldVideoModel,
} from "./higgsfieldMediaGeneration.mjs";
...
export const IMAGE_MODELS = [
  ...
  ...RUNWAY_IMAGE_MODELS.map(({ id, label, provider }) => ({ id, label, provider, requiresRunway: true })),
  ...HIGGSFIELD_IMAGE_MODELS.map(({ id, label, provider }) => ({ id, label, provider, requiresHiggsfield: true })),
];
...
export const VIDEO_MODELS = [
  ...
  ...RUNWAY_VIDEO_MODELS.map(({ id, label, provider }) => ({ id, label, provider, requiresRunway: true })),
  ...HIGGSFIELD_VIDEO_MODELS.map(({ id, label, provider }) => ({ id, label, provider, requiresHiggsfield: true })),
];
...
function normalizeImageModel(rawModel) {
  ...
  if (isLovartImageModel(lower) || isRunwayImageModel(lower) || isHiggsfieldImageModel(lower)) return lower;
...
function normalizeVideoModel(rawModel) {
  ...
  if (isLovartVideoModel(lower) || isRunwayVideoModel(lower) || isHiggsfieldVideoModel(lower)) return lower;
...
export async function generateImageMedia(input = {}) {
  ...
  const media = isLovartImageModel(model)
    ...
    : isRunwayImageModel(model)
      ? await generateRunwayImageMedia({ ...input, model })
      : isHiggsfieldImageModel(model)
        ? await generateHiggsfieldImageMedia({ ...input, model })
...
export async function generateVideoMedia(input = {}) {
  ...
  const media = isLovartVideoModel(model)
    ...
    : isRunwayVideoModel(model)
      ? await generateRunwayVideoMedia({ ...input, model })
      : isHiggsfieldVideoModel(model)
        ? await generateHiggsfieldVideoMedia({ ...input, model })
