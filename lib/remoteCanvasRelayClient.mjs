...
const CUSTOM_DATA_MAX_DEPTH = 6;
const CUSTOM_DATA_MAX_ARRAY_ITEMS = 50;
const CUSTOM_DATA_MAX_OBJECT_KEYS = 100;
const CUSTOM_DATA_MAX_STRING_CHARS = 20_000;
...
function compactCustomDataValue(value, depth = 0) {
  if (value === null || value === undefined) return value;
  const valueType = typeof value;
  if (valueType === "string") {
    if (value.startsWith("data:")) return undefined;
    if (value.length > CUSTOM_DATA_MAX_STRING_CHARS) {
      return `${value.slice(0, CUSTOM_DATA_MAX_STRING_CHARS)}...`;
    }
    return value;
  }
  if (valueType === "number" || valueType === "boolean") return value;
  if (valueType !== "object") return undefined;
  if (depth >= CUSTOM_DATA_MAX_DEPTH) return undefined;
  if (Array.isArray(value)) {
    const compacted = [];
    for (const item of value.slice(0, CUSTOM_DATA_MAX_ARRAY_ITEMS)) {
      const next = compactCustomDataValue(item, depth + 1);
  ...
  const compacted = {};
  for (const [key, item] of Object.entries(value).slice(0, CUSTOM_DATA_MAX_OBJECT_KEYS)) {
    if ((key === "dataURL" || key === "thumbnail") && typeof item === "string" && item.startsWith("data:")) {
      continue;
    }
    const next = compactCustomDataValue(item, depth + 1);
...
function compactCustomData(customData) {
  if (!customData || typeof customData !== "object" || Array.isArray(customData)) return undefined;
  const compacted = compactCustomDataValue(customData);
...
// Cloud URL out of band) and inlines only small base64 records. Generator
// metadata in customData is included so the Cloud canvas can show the same
// prompt panel, while bulky inline bitmaps/thumbnails are stripped.
function toEditorElement(element) {
  const { customData, ...rest } = element;
  const compacted = compactCustomData(customData);
  return compacted ? { ...rest, customData: compacted } : rest;
}
