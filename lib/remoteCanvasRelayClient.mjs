...
// verbatim (so the mobile canvas matches the desktop), plus a files map that
// references heavy bitmaps by asset name (resolved to a Cloud URL out of band)
// and inlines only small base64 records. Returns the scene and the list of
// asset names the viewer needs so the caller can ensure they are uploaded.
...
export function createRemoteCanvasRelayClient(options = {}) {
  ...
  async function uploadOneAsset(assetName) {
    ...
    if (cached && cached.size === fileStat.size) return cached.url;
  ...
  async function ensureAssetsUploaded(assetNames) {
    ...
    for (const name of assetNames) {
      try {
        const url = await uploadOneAsset(name);
        if (url) map[name] = url;
      } catch (error) {
  ...
  async function sendSnapshot({ force = false } = {}) {
    ...
    const snapshotCore = {
      ...
      scene: publicScene,
      // Full-fidelity scene + file map for a real Excalidraw viewer.
